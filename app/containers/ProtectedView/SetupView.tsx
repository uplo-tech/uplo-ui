...
function showConfirm() {
  confirm({
    title: 'Please Wait for Uplo-UI to Sync',
    content:
      'Uplo-UI must be fully synced with the network in order to restore your wallet. Sync status is displayed in the upper-right corner.',
    onOk() {}
  })
}
...
class SetupView extends React.Component<Props, {}> {
  ...
  render() {
    const { wallet, uplod, consensus } = this.props
    if (wallet.unlocked) {
      return <Redirect to="/" />
    }
    if (wallet.encrypted) {
