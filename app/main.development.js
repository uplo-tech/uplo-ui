  ...
  installExtensions().then(() => {
    ...
    mainWindow.on('close', e => {
      ...
      if (!mainWindow.isQuitting) {
        ...
        mainWindow.hide()
        return false
        // if uplod is shutdown, this flag will be true. we can quit safely.
      } else if (mainWindow.uplodShutdown) {
        return true
        // we can't quit yet, uplod is not shutdown. send an ipc event to
        // renderer so the shutdown process can start.
      } else {
        e.preventDefault()
        mainWindow.webContents.send('shutdown-uplod', true)
      }
    })
    ...
    appIcon.setToolTip('Uplo-UI syncs the daemon in the background.')
