...
import { getGlobalSiadProcess, uplod } from 'api/uplod'
...
export const SettingsModal: React.SFC<SettingModalprops> = ({ onOk, visible }) => {
  ...
  const uplodState = useSiadUIState()
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        if (!process.env.APPIMAGE) {
          ...
          if (globalSiadProcess) {
            uplod.daemonStop()
            let counter = 0
            // start a counter loop to test if Uplo is still running.
            setInterval(() => {
          ...
          // we are opting to use some bash magic to help relaunch Uplo-UI when
          // settings are udpated.
          // This process works by sending a safe shutdown request to the main
          // process, which triggers a shutdown of uplod and subsequently the UI.
  ...
  return (
    <div>
      ...
      >
        {!uplodState.isInternal && (
          <Box>
            <StyledCard bordered={false}>
              Please note that you are currently using an external instance of Uplo and some
          ...
          <SettingItem
            ...
            render={
              <TextInput
                id="consensusPath"
                value={config.uplod.datadir}
                onChange={e =>
                  setConfig({ ...config, uplod: { ...config.uplod, datadir: e.target.value } })
                }
              />
            }
          />
          <SettingItem
            title="Use Custom Uplod"
            render={
              <Switch
                checked={config.uplod.useCustomBinary}
                onChange={c =>
                  setConfig({ ...config, uplod: { ...config.uplod, useCustomBinary: c } })
                }
              />
            }
          />
          {config.uplod.useCustomBinary && (
            <SettingItem
              title="Daemon Path"
              render={
                <TextInput
                  id="daemonPath"
                  value={config.uplod.path}
                  onChange={e =>
                    setConfig({ ...config, uplod: { ...config.uplod, path: e.target.value } })
                  }
                />
              }
            />
          ...
          <SettingItem
            title="Use Custom Uploc"
            render={
              <Switch
                checked={config.uploc.useCustomBinary}
                onChange={c =>
                  setConfig({ ...config, uploc: { ...config.uploc, useCustomBinary: c } })
                }
              />
            }
          />
          {config.uploc.useCustomBinary && (
            <SettingItem
              title="Client Path"
              render={
                <TextInput
                  id="clientPath"
                  value={config.uploc.path}
                  onChange={e =>
                    setConfig({ ...config, uploc: { ...config.uploc, path: e.target.value } })
                  }
                />
              }
            />
