...
import { toHastings, toSiacoins } from 'uplo-typescript'
import {
  bytesToGBString,
  bytesToTBString,
  BLOCKS_PER_MONTH,
...
export const AllowanceModal = (props: any) => {
  ...
  const [uplodConstants, _] = useSiad('/daemon/constants')
  ...
  const initialFormValues = React.useMemo(() => {
    if (!uplodConstants.loading && uplodConstants.response) {
      const { defaultallowance } = uplodConstants.response
    ...
    return {}
  }, [renterSettings.allowance, uplodConstants])
  ...
  const showConfirm = React.useCallback(
    payload => {
      ...
      const message = `Are you sure you want to set a ${payload.allowance} UPLO allowance for ${
        payload.periodMonth
      } months to rent storage on Uplo?`
      StyledModal.confirm({
            ...
            <Box>
              <StyledTabs defaultActiveKey="1" tabPosition="left">
                <Tabs.TabPane tab="Basic" key="1">
                  <Box pb={3}>
                    <Text color="near-black">
                      To store files on Uplo, you must allocate funds in advance. If this is your
                      first time using Uplo-UI, the below settings are filled with default values.
                    </Text>
                  </Box>
                  <Box>
                ...
                <Tabs.TabPane tab="Advanced" key="2">
                  <Box pb={3}>
                    <Text color="near-black">
                      By setting accurate advanced settings, Uplo can better manage your contracts
