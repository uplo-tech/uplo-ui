...
import { initSiad, uplod, launchSiad, setGlobalSiadProcess, getGlobalSiadProcess } from 'api/uplod'
...
class OffState extends React.Component<DispatchProp> {
  ...
  launchDaemon = async () => {
    ...
    const isRunning = await uplod.isRunning()
    // If not running, we'll try to launch uplod ourselves
    if (!isRunning) {
      ...
      try {
        ...
        if (loaded) {
          ...
          dispatch(GlobalActions.uplodLoaded())
          dispatch(GlobalActions.startSiadPolling())
        } else {
          dispatch(GlobalActions.uplodOffline())
          this.setState({
            status: 'close-circle'
          })
        }
        ...
        >
          ...
          <Box mx={2} width={1 / 3} style={{ textAlign: 'center' }}>
            <Text is="p" color="mid-gray" fontSize={3}>
              {this.state.status === 'loading' ? 'Connecting' : 'Uplo Daemon Unreachable'}
            </Text>
            {this.state.status !== 'loading' && (
              <Text is="p" color="silver" fontSize={2} maxWidth="300em">
                It's likely the uplod process crashed or was never started. Please start uplod from
          ...
          {this.state.status !== 'loading' && (
            <Box>
              <Button onClick={this.launchDaemon}>Launch Uplo Daemon</Button>
            </Box>
          )}
        </Flex>
      </Flex>
