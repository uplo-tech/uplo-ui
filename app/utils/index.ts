...
export const groupTransactionsByDay = (group: TransactionGroup, count?: number) => {
  ...
  // https://github.com/uplo-tech/Uplo/issues/3257
...
export const isSetupTransaction = (tx: WalletModel.ProcessedTransaction): boolean => {
  ...
  const outputCheck = every(tx.outputs, (o: WalletModel.ProcessedOutput) =>
    o.fundtype === 'UploCoin output' ? o.walletaddress : true
  )
...
export const computeTxSum = (txn: WalletModel.ProcessedTransaction) => {
  ...
  if (txn.inputs) {
    ...
    totalSiacoinInput = sumCurrency(walletInputs, 'UploCoin')
    totalSiafundInput = sumCurrency(walletInputs, 'UploFund')
  ...
  if (txn.outputs) {
    ...
    totalSiacoinOuput = sumCurrency(walletOutputs, 'UploCoin')
    totalSiafundOutput = sumCurrency(walletOutputs, 'UploFund')
