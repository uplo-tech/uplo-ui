...
function* pollSiad() {
  ...
  while (true) {
    const running = yield call(uplod.isRunning)
    if (running) {
      yield put(GlobalActions.uplodLoaded())
      if (!notificationTriggered) {
        notificationTriggered = true
        yield put(
          GlobalActions.notification({
            title: 'Started Polling',
            message: 'Uplo-UI established a connection with Uplo',
            type: 'open'
          })
        )
      }
    } else {
      yield put(GlobalActions.uplodOffline())
    }
    yield delay(3000)
  }
}

function* uplodPoller() {
